...
PresetvCPUMemoryDisk spacemicro0.250.2510GBsmall-1x (default)0.50.510GBsmall-2x1110GBmedium-1x1210GBmedium-2x2410GBlarge-1x4810GBlarge-2x81610GB

You can view the Trigger.dev cloud pricing for these machines [here](https://trigger.dev/pricing#computePricing).

## 
[​
](#overriding-the-machine-when-triggering)Overriding the machine when triggering
...
### 
[​
](#memory/resource-monitoring)Memory/Resource monitoring
 ...
 */
 private async getProcMetrics(pids: number[]): Promise {
 return Promise.all(
 pids.map(async (pid) => {
 try {
 // Read process status
 ...
 */
 private async findPidsByName(processName?: string): Promise {
 if (!processName) {
 return [];
 }
 ...
 */
 async logResourceSnapshot(label = "Resource Snapshot"): Promise {
 try {
...
### 
[​
](#explicit-oom-errors)Explicit OOM errors
...
### 
[​
](#retrying-with-a-larger-machine)Retrying with a larger machine
...
[Suggest edits](https://github.com/triggerdotdev/trigger.dev/edit/main/docs/machines.mdx)[Raise issue](https://github.com/triggerdotdev/trigger.dev/issues/new?title=Issue on docs&body=Path: /machines)

[Previous](/docs/versioning)[IdempotencyAn API call or operation is “idempotent” if it has the same result when called more than once.
...
- [Overriding the machine when triggering](#overriding-the-machine-when-triggering)
- [Out Of Memory (OOM) errors](#out-of-memory-oom-errors)
- [Memory/Resource monitoring](#memory%2Fresource-monitoring)
- [Explicit OOM errors](#explicit-oom-errors)
- [Retrying with a larger machine](#retrying-with-a-larger-machine)
