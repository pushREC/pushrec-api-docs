...
[Log in](/login?returnTo=%2Fdocs%2Fen%2Fagent-sdk%2Ftypescript)
...
`string | AsyncIterable<`[`SDKUserMessage`](#sdkusermessage)`>`
...
`(args, extra) => Promise<`[`CallToolResult`](#calltoolresult)`>`
...
Array of tool definitions created with [`tool()`](#tool)
...
`Record<string, [`AgentDefinition`](#agentdefinition)>`
...
`rewindFiles(userMessageUuid)`

Restores files to their state at the specified user message. Requires `enableFileCheckpointing: true`. See [File checkpointing](/docs/en/agent-sdk/file-checkpointing)
...
Enables 1 million token [context window](/docs/en/build-with-claude/context-windows)
...
Enable file change tracking for rewinding. See [File checkpointing](/docs/en/agent-sdk/file-checkpointing)
...
`Partial<Record<`[`HookEvent`](#hookevent)`,` [`HookCallbackMatcher`](#hookcallbackmatcher)`[]>>`
...
Define output format for agent results. See [Structured outputs](/docs/en/agent-sdk/structured-outputs) for details
...
Load custom plugins from local paths. See [Plugins](/docs/en/agent-sdk/plugins) for details
...
Configure sandbox behavior programmatically. See [Sandbox settings](#sandboxsettings) for details
...
Control which filesystem settings to load. When omitted, no settings are loaded. **Note:** Must include `'project'` to load CLAUDE.md files
...
`undefined` (empty prompt)

System prompt configuration. Pass a string for custom prompt, or `{ type: 'preset', preset: 'claude_code' }` to use Claude Code's system prompt. When using the preset object form, add `append` to extend the system prompt with additional instructions
...
Allow the model to request running commands outside the sandbox. When `true`, the model can set `dangerouslyDisableSandbox` in tool input, which falls back to the [permissions system](#permissions-fallback-for-unsandboxed-commands)

`network`

[`NetworkSandboxSettings`](#networksandboxsettings)
...
SOCKS proxy port for network reques
