...
duration_ms: 4065
---
gent SDK reference - Python - Claude Docs
...
[Log in](/login?returnTo=%2Fdocs%2Fen%2Fagent-sdk%2Fpython)
...
Optional configuration object (defaults to `ClaudeAgentOptions()` if None)
...
List of tool functions created with `@tool` decorator
...
Restore files to their state at the specified user message. Requires . See
...
System prompt configuration. Pass a string for custom prompt, or use `{"type": "preset", "preset": "claude_code"}` for Claude Code's system prompt. Add `"append"` to extend the preset
...
Must be `"json_schema"` for JSON Schema validation
...
Must be `"preset"` to use a preset system prompt

`preset`

Yes

Must be `"claude_code"` to use Claude Code's system prompt
...
_Deprecated_ - File-like object for debug output. Use `stderr` callback instead
...
Tool permission callback function
...
[`SandboxSettings`](#sandboxsettings) `| None`
