...
duration_ms: 1558
---
odel configuration - Claude Code Docs
...
Uses the latest Sonnet model (currently Sonnet 4.5) for daily coding tasks

**`opus`**

Uses Opus model (currently Opus 4.5) for specialized complex reasoning tasks
...
Uses Sonnet with a [1 million token context window](https://docs.claude.com/en/docs/build-with-claude/context-windows#1m-token-context-window) window for long sessions
...
The model to use for `haiku`, or [background functionality](/docs/en/costs#background-token-usage)

`CLAUDE_CODE_SUBAGENT_MODEL`

The model to use for [subagents](/docs/en/sub-agents)
