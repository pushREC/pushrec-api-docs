---
source: https://docs.digitalocean.com/products/app-platform/getting-started/sample-apps/python/
scraped: 2026-01-07
...
The Python buildpack applies to this sample app. See the [Python buildpack overview](/products/app-platform/reference/buildpacks/python/) for more information about its configuration options and limitations.

> This content is automatically generated from [https://github.com/digitalocean/sample-python/blob/main/README.md](https://github.com/digitalocean/sample-python/blob/main/README.md).
...
*   Gunicorn crashes with `PermissionError` unless launched with a specific argument due to an issue with Gunicorn running in Docker. To resolve, run Gunicorn to use a different location for its temp files by passing this option to the run command: `gunicorn --worker-tmp-dir /dev/shm project.wsgi`, where `project` is the name of your project.