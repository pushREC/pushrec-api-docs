...
source_url: https://docs.digitalocean.com/products/networking/reserved-ips/details/features/
domain: docs.digitalocean.com
---
                Reserved IP Features | DigitalOcean Documentation
...
For example, you can assign a reserved IP to a Droplet that is running a web application. You can then write a script that uses the [DigitalOcean Monitoring API](/reference/api/digitalocean/#tag/Monitoring) to monitor the Droplet’s performance and then reassign the IP address to a backup Droplet ([using the API](/reference/api/digitalocean/#tag/Reserved-IPs)) if the original Droplet’s performance becomes unstable or poor.
