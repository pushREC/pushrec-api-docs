---
source: https://docs.digitalocean.com/products/databases/kafka/how-to/
scraped: 2025-01-07
tags: [digitalocean, databases, managed-db, api-docs]
---

                Kafka How-Tos | DigitalOcean Documentation

*   [
    
    DigitalOcean | Docs](https://docs.digitalocean.com/)

*   

*   [Platform](https://docs.digitalocean.com/platform/)
*   [Products](https://docs.digitalocean.com/products/)
*   [Reference](https://docs.digitalocean.com/reference/)
*   [Support](https://docs.digitalocean.com/support/)
*   [Sign Up](https://cloud.digitalocean.com/registrations/new)

*   [![](/images/icons/kafka.6e5f03a9446ce8ef1cb8ff94d5cfc2174c6a0778b884650e64476af3a9d0888b.svg)Kafka](/products/databases/kafka/)
*   [Getting Started](/products/databases/kafka/getting-started/)
    *   [Quickstart](/products/databases/kafka/getting-started/quickstart/)
*   [How-Tos](/products/databases/kafka/how-to/)
    *   [Create Kafka Clusters](/products/databases/kafka/how-to/create/)
    *   [Connect to Kafka Clusters](/products/databases/kafka/how-to/connect/)
    *   [Create Kafka Topics](/products/databases/kafka/how-to/create-topics/)
    *   [Resize Database Clusters](/products/databases/kafka/how-to/resize/)
    *   [Secure Kafka Clusters](/products/databases/kafka/how-to/secure/)
    *   [Schedule Automatic Updates](/products/databases/kafka/how-to/schedule-updates/)
    *   [Manage Users and Privileges](/products/databases/kafka/how-to/manage-users-and-privileges/)
    *   [Monitor Kafka Performance](/products/databases/kafka/how-to/monitor-databases/)
    *   [Monitor Cluster Performance](/products/databases/kafka/how-to/monitor-clusters/)
    *   [Set Up Monitoring Alerts](/products/databases/kafka/how-to/set-up-alerts/)
    *   [Forward Logs](/products/databases/kafka/how-to/forward-logs/)
    *   [Destroy Clusters](/products/databases/kafka/how-to/destroy/)
    *   [Enable Schema Registry](/products/databases/kafka/how-to/enable-schema-registry/)
    *   [Tag Database Clusters](/products/databases/kafka/how-to/tag/)
    *   [Reconfigure Database Clusters](/products/databases/kafka/how-to/reconfigure/)
*   [Reference](/products/databases/kafka/reference/)
    *   [API Reference](https://docs.digitalocean.com/reference/api/digitalocean/#tag/Databases)
    *   [CLI Reference](/reference/doctl/reference/databases/)
    *   [MCP Reference](/reference/mcp/)
*   [Concepts](/products/databases/kafka/concepts/)
    *   [Best Practices](/products/databases/kafka/concepts/best-practices/)
    *   [Shared Responsibility Model](/products/databases/kafka/concepts/responsibility-model/)
*   [Details](/products/databases/kafka/details/)
    *   [Features](/products/databases/kafka/details/features/)
    *   [Pricing](/products/databases/kafka/details/pricing/)
    *   [Availability](/products/databases/kafka/details/availability/)
    *   [Limits](/products/databases/kafka/details/limits/)
    *   [Cluster Notifications](/products/databases/kafka/details/notifications/)
    *   [Service-Level Agreement](https://www.digitalocean.com/sla/databases)

*   How-Tos

[Give Feedback](https://ideas.digitalocean.com/documentation)

# Kafka How-Tos

Generated on 6 Jan 2026

Kafka is an open-source distributed event and stream-processing platform built to process demanding real-time data feeds. It is inherently scalable, with high throughput and availability.

## Getting Started[](#getting-started)

[](/products/databases/kafka/how-to/create/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Create Kafka Clusters

Create a Kafka cluster from the DigitalOcean Control Panel.

[](/products/databases/kafka/how-to/connect/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Connect to Kafka Clusters

Connect to Kafka clusters from the command line or other applications.

[](/products/databases/kafka/how-to/create-topics/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Create Kafka Topics

Create a Kafka Topic from the DigitalOcean Control Panel.

[](/products/databases/kafka/how-to/secure/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Secure Kafka Managed Clusters

Add additional security to a Kafka managed database cluster by restricting incoming connections and using encrypted connections.

[](/products/databases/kafka/how-to/schedule-updates/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Schedule Automatic Software Updates for Kafka Clusters

Customize the maintenance window for automatic software updates to your database cluster.

[](/products/databases/kafka/how-to/tag/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Tag Kafka Clusters

Organize Kafka databases clusters with tags to group and filter databases or create monitoring alert policies for multiple databases at once.

## Cluster Infrastructure[](#cluster-infrastructure)

[](/products/databases/kafka/how-to/resize/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Resize Kafka Clusters

Increase the size of your database cluster to accommodate your data needs.

[](/products/databases/kafka/how-to/monitor-databases/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Monitor Kafka Database Performance

Access and interpret Kafka database performance metrics.

[](/products/databases/kafka/how-to/monitor-clusters/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Monitor Kafka Database Cluster Performance

Access and understand performance metrics for nodes in a database cluster.

[](/products/databases/kafka/how-to/set-up-alerts/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Set Up Kafka Cluster Monitoring Alerts

Configure alert policies based on database cluster performance metrics.

[](/products/databases/kafka/how-to/forward-logs/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Forward Logs from a Kafka Cluster

Forward database logs to OpenSearch, Elasticsearch, or Rsyslog.

## Kafka Management[](#kafka-management)

[](/products/databases/kafka/how-to/manage-users-and-privileges/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Manage Kafka Users in a Database Cluster

Create and delete a database cluster’s databases and database users from the DigitalOcean Control Panel.

[](/products/databases/kafka/how-to/reconfigure/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Reconfigure Kafka Database Clusters

Edit your database engine’s configuration and parameters via the API.

## Cluster Management[](#cluster-management)

[](/products/databases/kafka/how-to/enable-schema-registry/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Enable Schema Registry on Kafka Clusters

Enable schema registry to protect your cluster when sending or receiving incorrect, modified, or corrupt data.

[](/products/databases/kafka/how-to/destroy/)

![](https://docs.digitalocean.com/images/icons/kafka.svg)

How to Destroy Kafka Clusters

Destroy a database cluster to permanently and irreversibly destroy the cluster, its contents, and its automated backups.

In this article...

[Kafka How-Tos](https://docs.digitalocean.com/products/databases/kafka/how-to/)

##### Company

*   [About](https://www.digitalocean.com/about)
*   [Careers](https://www.digitalocean.com/careers)
*   [Blog](https://www.digitalocean.com/blog)

##### Docs

*   [Docs Home](https://docs.digitalocean.com)
*   [API Reference](https://docs.digitalocean.com/reference/api)
*   [CLI Reference](https://docs.digitalocean.com/reference/doctl)
*   [Release Notes](https://docs.digitalocean.com/release-notes)
*   [Trust Platform](https://www.digitalocean.com/trust)

##### Community

*   [Tutorials](https://www.digitalocean.com/community/tutorials)
*   [Q&A](https://www.digitalocean.com/community/questions)
*   [Write for DOnations](https://www.digitalocean.com/community/pages/write-for-digitalocean)
*   [Currents Research](https://www.digitalocean.com/currents)
*   [Legal](https://www.digitalocean.com/legal)
*   [Code of Conduct](https://www.digitalocean.com/community/pages/code-of-conduct)

##### Support

*   [Support Center](/support)
*   [Report Abuse](https://www.digitalocean.com/company/contact/abuse)

* * *

© 2025 DigitalOcean, LLC. All rights reserved

[](https://x.com/digitalocean)[](https://www.instagram.com/thedigitalocean)[](https://www.facebook.com/DigitalOceanCloudHosting)[](https://discord.gg/digitalocean)[](https://www.youtube.com/DigitalOcean)[](https://www.linkedin.com/company/digitalocean)[](https://github.com/digitalocean)

### We can't find any results for your search.

Try using different keywords or simplifying your search t
