---
source: https://docs.digitalocean.com/products/app-platform/how-to/manage-workers/
scraped: 2026-01-07
...
Background workers can consist of any application code you like. As far as App Platform is concerned, they are the same as [services](/products/app-platform/how-to/manage-services/), except they are not routable, meaning they are not accessible via an external URL.

## Add a Worker to an App Using Automation[](#add-a-worker-to-an-app-using-automation)

You can add a worker to an app using the CLI’s app update command or the API’s app update endpoint. To add a worker, update the [app’s spec](/products/app-platform/reference/app-spec/) with the worker’s specifications and submit the spec using the following command or endpoint.
...
### cURL[](#add-a-worker-to-an-app-curl)
...
### Python[](#add-a-worker-to-an-app-python)
...
## Add a Worker Using the Control Panel[](#add-a-worker-using-the-control-panel)

In the [**Apps** section of the DigitalOcean Control Panel](https://cloud.digitalocean.com/apps), select your app. Click **Add components**, then choose **Create resources from source code** to add a Worker.
...
## Destroy Workers[](#destroy-workers)
